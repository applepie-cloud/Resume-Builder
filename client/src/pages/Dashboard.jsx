    ...
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        <p className='text-2xl font-medium mb-6 bg-linear-to-r from-slate-600 to-slate-800 bg-clip-text text-transparent sm:hidden'>
          Welcome, Aditya Verma
        </p>

        <div className='flex gap-4'>
          <button onClick={() => setShowCreateResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center gap-2 
          rounded-lg text-slate-600 border border-dashed border-slate-300 group hover:border-indigo-500 transition-all duration-300 cursor-pointer hover:shadow-lg'>
          ...
          <button onClick={() => setShowUploadResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center gap-2 
          rounded-lg text-slate-600 border border-dashed border-slate-300 group hover:border-purple-500 transition-all duration-300 cursor-pointer hover:shadow-lg'>
        ...
        <hr className='border-slate-300 my-8 sm:w-76.25' />
          ...
          {allResumes.map((resume, index) => {
            ...
            return (
              <button 
                ...
                key={index} 
                className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center gap-2 rounded-lg border group hover:shadow-lg transition-all duration-300 cursor-pointer' 
                style={{
            ...
            <form onSubmit={createResume} onClick={() => setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
              <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                <h2 className='text-xl font-bold mb-4'>Create a Resume</h2>
                 <input onChange={(e) => {setTitle(e.target.value)}} value={title} type="text" placeholder='Enter resume title' className='border focus:border-green-300 rounded px-4 py-2 w-full ring-green-600' required/>
              ...
              <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                ...
                <div >
                  <label htmlFor='resume-input' className='block text-sm text-slate-700'>
                    Select Resume File
                    <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed
                    rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'>
                      ...
                      {resume ? (
                        ...
                        <>
                          <UploadCloud className='size-10 storke-1'/>
                          <p> upload resume </p>
                        </>
                      )}
                    </div>
                  ...
                  <input type='file' id='resume-input' hidden accept='.pdf,.doc,.docx' onChange={(e) => setResume(e.target.files[0])} required/>
                </div>
