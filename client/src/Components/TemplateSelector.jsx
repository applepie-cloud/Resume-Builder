...
const TemplateSelector = ({ selectedTemplate, onChange }) => {
    ...
    const templates = [
        ...
        },
        {
            id: 'minimal',
            name: 'Minimal',
            preview: 'A minimalist design with ample white space and simple typography'
        },
        {
            id: 'minimal-image',
            name: "Minimal Image",
  ...
  return (
    <div className='relative'>
        <button className='flex items-center gap-1 text-sm text-blue-600 bg-grdient-to-br from-blue-50 to-blue-100 ring-blue-300 hover:ring transition-all px-3 py-3 rounded-lg' 
            ...
            <div className='absolute top-full w-xs p-3 mt-2 space-y-3 z-10 bg-white rounded-md border border-gray-200 shadow-sm'>
                {templates.map((template) => (
                    <div key={template.id} onClick={() => {onChange(template.id);setIsOpen(false)}} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? 'border-blue-400 bg-blue-100' : 'border-gray-300 hover:border-gray-400 hover:bg-gray-100'}`}> 
                        {selectedTemplate === template.id && (<div className='absolute top-2 right-2'>
                                <div>
